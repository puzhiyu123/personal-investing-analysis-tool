...
interface ScanWatchlistButtonProps {
  onComplete: () => void;
}

export function ScanWatchlistButton({ onComplete }: ScanWatchlistButtonProps) {
  ...
  const pollScan = useCallback(
    (scanId: string) => {
      pollRef.current = setInterval(async () => {
        try {
          const res = await fetch(`/api/watchlist/scan/${scanId}`);
          if (!res.ok) return;
